...
def _apply_augmentation(
    df: "pd.DataFrame", columns_to_augment: List[str], augmenting_func: "function", inflate_factor: int = 3
) -> "pd.DataFrame":
...
def _apply_augmentation_on_single_data_point(
    df: "pd.DataFrame", columns_to_augment: List[str], augmenting_func: "function", magnitude: float
) -> "pd.DataFrame":
    not_augmented_cols = [i for i in df.columns.to_list() if i not in columns_to_augment]
    augmented_rows = {item: [] for item in df.columns.to_list()}
    ...
    for _, row in df.iterrows():
        for col in columns_to_augment:
            if row[col] is None:
                ...
                sentence = str(row[col]).split(" ")
                augmented_sentence = augmenting_func(sentence, magnitude)
                augmented_sentence = " ".join(augmented_sentence)
                augmented_rows[col].append(augmented_sentence)
...
def delete_random_words(df: "pd.DataFrame", columns_to_augment: List[str], inflate_factor: int = 3) -> "pd.DataFrame":
    return _apply_augmentation(
        df=df, columns_to_augment=columns_to_augment, augmenting_func=_random_deletion, inflate_factor=inflate_factor
    )


def replace_with_synonyms(df: "pd.DataFrame", columns_to_augment: List[str], inflate_factor: int = 3) -> "pd.DataFrame":
    return _apply_augmentation(
        df=df, columns_to_augment=columns_to_augment, augmenting_func=_synonym_replacement, inflate_factor=inflate_factor
    )


def swap_random_words(df: "pd.DataFrame", columns_to_augment: List[str], inflate_factor: int = 3) -> "pd.DataFrame":
    return _apply_augmentation(
        df=df, columns_to_augment=columns_to_augment, augmenting_func=_random_swap, inflate_factor=inflate_factor
    )


def insert_random_words(df: "pd.DataFrame", columns_to_augment: List[str], inflate_factor: int = 3) -> "pd.DataFrame":
    return _apply_augmentation(
        df=df, columns_to_augment=columns_to_augment, augmenting_func=_random_insertion, inflate_factor=inflate_factor
    )
