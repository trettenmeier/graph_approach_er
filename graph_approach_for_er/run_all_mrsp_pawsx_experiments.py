...
from graph_approach_for_er.utils.load_config import load_global_config
...
class RunAllExperimentsTask(LuigiBaseTask):
    ...
    def requires(self):
        directory_path = os.path.abspath(graph_approach_for_er.experiments.__file__).replace("__init__.py", "")
    ...
    def get_filenames_without_extension(directory_path):
        ...
        return [i for i in filenames if ("__init__" not in i and "__pycache__" not in i and ("pawsx" in i or "mrsp" in i))]
